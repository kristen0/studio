...
export function InventoryPageClient() {
  ...
  const filteredItems = useMemo(() => {
    let visibleItems = items.filter(item => {
      ...
      // Hide items that expired more than 5 days ago unless we are filtering for expired
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div className="w-full flex-grow text-left">
                ...
                <p className="text-muted-foreground text-sm md:text-base">
                    {items.length > 0 ? `You have ${items.filter(i => i.status !== 'OUT_OF_STOCK').length} items in your inventory.` : 'Your inventory is empty.'}
                </p>
            </div>
        ...
        {loading ? (
          ...
          <div className="flex h-[50vh] w-full flex-col items-center justify-center rounded-lg border-2 border-dashed border-border/20 text-center p-4">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              {searchQuery ? `No items match your search for "${searchQuery}".` : statusFilter === 'ALL' ? "Your inventory is empty. Add an item to get started." : `No items match the filter "${statusFilter.replace(/_/g, ' ')}".`}
            </p>
          </div>
        ) : (
